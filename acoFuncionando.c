...
void printItemList(ITEM_NODE *head){
    ...
    if(head->next != NULL){
	    ...
	    while(1) {
            //printf(" - run while\n");
            countItens++;
            printf(" %d - p: %d, w: %d, r: %f | cand: %d\n", countItens, currentNode->item->profit, currentNode->item->weight, currentNode->item->pheromone, currentNode);
            if(currentNode->next == NULL){
                break;
            }
            currentNode = currentNode->next;
...
void printItens(ITEM *itens, int nItens){
    ...
    printf("\n ------------| TODOS OS ITENS: |------------\n");
    for(i = 0; i < nItens; i++){
        printf(" | %d - p: %d,\tw: %d,\tr: %lf\t| cand: %d\n", i, itens[i].profit, itens[i].weight, itens[i].pheromone, &itens[i]);
    }
    printf(" ------------| Total de itens: %d |------------\n\n", i);
}
...
double probProduct(ITEM item){
    double product;
	double quotient = ((double)item.profit / (double)item.weight);
	...
	while(countCycles < nCycles){
		while(countAnts < nAnts){
			while(currentC > 0 && nNeighbours != 0){
				...
				solution.profit += candidate->profit;
		...
		countCycles++;
		printf(" Best solution till cycle %d:\t %d\n",countCycles, BEST.profit);
	}
	free(solution.itens);
}
/*
